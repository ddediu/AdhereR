...
As pre-requisites, the worker machines should allow [`SSH`](https://linuxconfig.org/linux-setup-ssh) access (for easiness, we use here *passwordless SSH access* from the "master"; see for example [here](http://www.linuxproblem.org/art_9.html) for this setup) and should have the `snow` package installed in `R`.
