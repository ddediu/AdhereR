...
In fact, it is conceivable that a **proportion** of the maximum permissible gap (and not necessarily the entire maximum permissible gap) is included.
To ver these scenarios, `compute.treatment.episodes()` has a parameter, `maximum.permissible.gap.append.to.episode.proportion` which represents the proportion of the maximum permissible gap included in the episodes: 0.0 (the default) means that the episodes end as described above), 1.0 means that 100% of maximum permissible gap is appended at the end of the episodes, while intermediate values specify what percentage (`maximum.permissible.gap.append.to.episode.proportion`*100%) of the maximum permissible gap is appended at the end of the episodes.
