---
title: "Calling `AdhereR` from `Python 3` and `Stata`"
author: "Dan Dediu"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Calling AdhereR from Python3 and Stata}
...
We present here this generic mechanism using its *reference implementation* for `Python 3`, and we apply it also to `Stata`.
While this reference implementation is definitely usable in production environments (including from [Jupyter Notebooks](#from-a-jupyter-notebook)) and comes with the `R` `AdhereR` package (see [here](#making-the-adherer-module-visible-to-python-aka-installation) on how to "install" it in `Python 3`), this can probably be improved both in terms of calling and passing data between `Python` and `R`, as well as in terms of the "pythonicity" of the `Python` side of the implementation.
...
Likewise, the `.ado` file (containing the `adherer` program) for `Stata` is a fully working implementation that can be used in daily work, but is probably far from optimal.
...
## Table of Contents
  ...
  - [The `Python 3` wrapper: the `adherer` module](#the-python-3-wrapper-the-adherer-module)
      ...
      - [Making the `adherer` module visible to Python (aka installation)](#making-the-adherer-module-visible-to-python-aka-installation)
  ...
  - [The `Stata` wrapper: the `adherer` program](#the-stata-wrapper-the-adherer-program)
  
      - [Making the `adherer` program visible to Stata (aka installation)](#making-the-adherer-program-visible-to-stata-aka-installation)
      
      - [The `adherer` program](#the-adherer-program)
      
      - [Main arguments and conventions](#main-arguments-and-conventions)
...
The argument `callig.platform` will allow the selection of the desired wrapper (now it is implicitly set to `python3`).

This file can be either:

  - *copied and placed* in `Python`'s "module search paths" (a list of directories comprising, in order, the directory containing the input script, the environment variable `PYTHONPATH`, and a default location; see [here](https://docs.python.org/3/tutorial/modules.html#the-module-search-path) for details), in which cae it can be simply imported using the standard `Python` syntax (e.g. `import adherer` or `import adherer as ad`), or
...
Here, we implemented this through the following class hierarchy (image generated with [pyreverse](https://pypi.org/project/pyreverse/), not showing the private attributes):
...
`AdhereR` is very easy to use from within a [Jupyter Notebook](https://jupyter.org/) (the only tricky bit being making sure that `adherer` module is visible to the `Python` kernel, but this is explained in detail in section [Making the `adherer` module visible to Python (aka installation)](#making-the-adherer-module-visible-to-python-aka-installation)).
...
While this is very easy and transparent, some people might prefer to use the more generic mechanism provided by [`rpy2`](https://rpy2.github.io/index.html), which, in principle, allows the transparent call of `R` code from `Python`.
In fact, `AdhereR` seems to play very nicely with `rpy2`, even within a [Jupyter Notebook](https://jupyter.org/), as shown by the code below (the full notebook is available in the `jupyter_notebook_python3_py2.ipynb` file accompanying the package in the same directory as the `adherer` module):
...
`AdhereR` uses `R`'s parallel processing capacities to split expensive computations and distribute them across multiple CPUs/cores in a single computer or even across a network of computers.
...
As pre-requisites, the worker machines should allow [`SSH`](https://ubuntu.com/server/docs/service-openssh) access (for easiness, we use here *passwordless SSH access* from the "master"; see for example [here](http://www.linuxproblem.org/art_9.html) for this setup) and should have the `snow` package installed in `R`.
...
While this is a very good way to transparently distribute processing to more powerful nodes over a network, there are several (potential) issues one must be aware of:
  ...
  - *you need to wait for the results*: this process assumes that the "master" will wait for the "workers" to finish and return their results; thus, putting the "master" to sleep, shutting it down or disconnecting it from the network will probably result in not being able to collect the resuls back. If one needs very long computations (say 3+ hours), offline mobility or the network is unreliable, we would suggest setting up a separate compute process (that may itself parallelise computations) on the remote machines using, for example, [`screen`](https://www.howtoforge.com/linux_screen), [`nohup`](https://en.wikipedia.org/wiki/Nohup) or a more specialised cluster management platform such as [Son of a Grid Engine (SGE)](https://github.com/daimh/sge).


## The `Stata` wrapper: the `adherer` program

Please note that, for reasons of access to a `Stata` license, we developed and tested this wrapper on `Windows 10` and with `Stata MP` version `13.1` *only*; while this might work on other platforms and versions (as it is or with some slight changes) we cannot currently ascertain it.

This wrapper is based on initial work by *Maria Rubio-Valera* during a research visit to Lyon in 2018, and on ideas and code from *Julian Reif*'s and *David Molitor*'s [`RSCRIPT` package](https://github.com/reifjulian/rscript) (v1.1), released under a license compatible with GPLv3 (https://www.gnu.org/licenses/license-list.html#GPLCompatibleLicenses; see "Expat License").

### Making the `adherer` program visible to Stata (aka installation)

The reference implementation is contained in single file (`adherer.ado`) included with the `R` `AdhereR` package and whose location can be obtained using the function `getCallerWrapperLocation("stata", full.path=TRUE)` from `AdhereR` (*N.B.* it is located in the directory where the `AdhereR` package is installed on the system, subdirectory `wrappers/stata/adherer.ado`; for example, on the example `Windows 10` machine this is `C:/Users/ddedi/Documents/R/win-library/4.1/AdhereR/wrappers/stata/adherer.ado`).
However, this `.ado` file is accompanied by several other files, including a `ReadMe.md` document with details over the 3rd party code and ideas used and licensing, the example `CSV` dataset used in the `Stata` examples (`test-data.csv`), as well as other files relevant for defining a `Stata` package (`adherer.pkg`, 'stata.toc' and the help file `adherer.hlp`).

The "normal" way of making `Stata` aware of a new package is by placing the necessary files in the **PERSONAL** `Stata` folder (which can be found using the `sysdir` command in `Stata`; see also [here](https://www.stata.com/support/faqs/programming/personal-ado-directory/)) -- if you want to do this, please copy at least the `adherer.ado` file there.

Alternatively, one may point `Stata` to the folder where this script resides in the package, using (on my test machine):

```
sysdir set PERSONAL "C:/Users/ddedi/Documents/R/win-library/4.1/AdhereR/wrappers/stata"
```

but this might make other personal packages that might exist in the "real" **PERSONAL** `Stata` folder not available any more.

This `adherer.ado` file implements several "programs", but the important one is the `adherer` program, which is the only one you should use to access `AdhereR`.

In addition, if you with to see the `Stata` help document in `Stata`, you can open the `adherer.hlp` file (located in the same folder as the `adherer.ado` file) using the File &rarr; Open ... menu.

*NOTE:* we will consistently use `AdhereR` to refer to the `R` package, and `adherer` to refer to the `Stata` program.


### The `adherer` program

First, let's make `Stata` aware of this by "running" the `adherer.ado` file:

```
. do "C:/Users/ddedi/Documents/R/win-library/4.1/AdhereR/wrappers/stata/adherer.ado"
```

Now, the `adherer` program should be callable from `Stata`. 
Please see either the `adherer.ado` file itself, or load the `adherer.hlp` help document in `Stata` for an overview of its arguments and usage.

In a nutshell, when invoked, this program does the following:

1. perform basic sanity checks (including if it is running on Windows and *not* in batch mode[^stata_batch]),

2. try to detect the default `R` on the machine,

3. check if `R` can be called and works correctly,

4. check if `AdhereR` is installed in `R` and has the right version,

5. perform various checks on the given arguments, build the parameters for calling `AdhereR` and make sure the data file(s) are in the right location (either the user-given folder or `Stata`'s own temporary directory),

6. call `AdhereR` and check the output,

7. if all seems ok, load the results in `Stata` (if the user so wishes) and display a confirmation message,

8. exit cleanly.

[^stata_batch]: There is an issue inherited from the `RSCRIPT` package, so that it does not work in batch mode on `Stata` for `Windows` because `Stata` ignores shell requests in this context (i.e., we cannot call `R`).

All these are done transparently for the `Stata` user (except for some `Windows` `cmd` windows flashing by).


### Main arguments and conventions

Most arguments of the `adherer` program map pretty transparently on the [communication protocol defined below](#appendix-i-the-communication-protocol) and the parameters of the `AdhereR` `CMA` and `plot` functions, with some observations:

- the optional argument `using/` allows, if present, the user to specify a `CSV` file containing the data; if missing (the default) the program will attempt to use the dataset already loaded in memory (if any)

- there are two types of arguments: presence/absence and of type "string":

  + the former are basically boolean flags that negate, when explicitly given, the default value assumed in their explicit absence (e.g., passing `plot` means that a plot should also be produced, while not passing it -- the default -- results in no plot),
  
  + the second encapsulate, for commodity, all arguments that can have non-boolean values (strings, numbers, values from a predefined set)
  
- `cma` specifies the function to compute: "CMA0" (can only be plotted), "CMA1", "CMA2", "CMA3", "CMA4", "CMA5", "CMA6", "CMA7", "CMA8", "CMA9", "CMA_per_episode", "CMA_sliding_window", "compute_event_int_gaps", "compute_treatment_episodes" and "plot_interactive_cma"

- `plot`: requests plotting

- `folder_to_use`: if given, must be an already existing folder (directory) with read and write access, where all the input and output data will be read from and written into; if not give, we'll use `Stata`'s own temporary directory

- `dont_load_results`; if absent (by default) the results of the estimation (normally, CMAs) are loaded in `Stata`, overwriting any pre-existing dataset, but if explicitly given, any pre-existing in-memory dataset is preserved and the results are left on disk.

Pease see also the `Stata` help file for more details.


### Some examples

Here I assume that the example dataset was loaded in `Stata` and that the `adherer.ado` file has been executed:

```
. import delimited "C:/Users/ddedi/Documents/R/win-library/4.1/AdhereR/wrappers/stata/test-dataset.csv", varnames(1) delimiter(tab) clear
. describe
...
- compute `CMA1` for all patients and load the results in `Sata`:

```
. adherer , verbose cma("CMA1") id_col("id") ev_date_col("date") ev_dur_col("duration")

Using R at location: C:/Program Files/R/R-4.1.3/bin/Rscript.exe
Running R script: C:\Users\ddedi\AppData\Local\Temp\ST_00000003.tmp
Args: "C:\Users\ddedi\AppData\Local\Temp\ST_00000009.tmp"
Begin R output:
...
________________________________________________________________________________
...end R output
CSV data for AdhereR is in file C:\Users\ddedi\AppData\Local\Temp\ST_00000005.tmp
Using R at location: C:/Program Files/R/R-4.1.3/bin/Rscript.exe
Running R script: C:\Users\ddedi\AppData\Local\Temp\ST_00000003.tmp
Args: "C:\Users\ddedi\AppData\Local\Temp\ST_00000007.tmp" "C:\Users\ddedi\AppData\Local\Temp\ST_00000005.tmp" "C:\Users\ddedi\AppData\Local\Tem
> p\ST_00000009.tmp"
Begin R output:
________________________________________________________________________________
stdout:
...
...end R output
(1 var, 1 obs)
Apprently, all went well: the estimated CMAs should have been loaded in STATA as a dataset with 100 observations and columns: id cma :
...
- compute and plot `CMA1` per sliding windows with duration 60 days and step 90 days:
...
- open the interactive plotting of the dataset in the default web browser (make sure the package `AdhereRViz` is also installed in `R`):
...
| `medication.groups` | NO | `""` | [a named vector of medication group definitions, the name of a column in the data that defines the groups, or ''; the medication groups are flattened into column __MED_GROUP_ID] |
