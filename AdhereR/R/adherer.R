                                     ...
                                     suppress.warnings=FALSE)
{
  ...
  safe_env <- new.env(parent = emptyenv());
  # ... add the safe functions:s
  for( .f in c(getGroupMembers("Math"), getGroupMembers("Arith"), getGroupMembers("Logic"), getGroupMembers("Compare"), "(", "[", "!") )
  {
  ...
  for( i in 1:nrow(mg) )
  {
    ...
    try(res <- eval(parse(text=paste0(mg$fnc_name[i],"()")), env=safe_env), silent=TRUE); # the errors will be anyway recorded in the .errs variable in safe_env
