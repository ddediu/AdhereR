...
)
{
  ...
  {
    if( !suppress.warnings ) .write.ewsn("The line point coodinates must be of the same length >= 2.\n", "warning", ".SVG.lines", "AdhereR");
    if( return_string ) return ("") else return (NULL);
  }
  ...
  } else
  {
    ...
    if( length(x) %% 2 != 0 )
    {
      if( !suppress.warnings ) .write.ewsn("For unconnected lines there must an even number of point coordinates.\n", "warning", ".SVG.lines", "AdhereR");
      return (NULL);
    }
...
)
{
  ...
  {
    if( !suppress.warnings ) .write.ewsn("There must be at least on point.\n", "warning", ".SVG.points", "AdhereR");
    return (NULL);
  }
...
)
{
  ...
  {
    if( !suppress.warnings ) .write.ewsn("There must be at least one text and the number of texts should matche the number of coordinates.\n", "warning", ".SVG.text", "AdhereR");
    return (NULL);
  }
...
map.event.coords.to.plot <- function(x=NA, y=NA, x.is.Date=FALSE, x.date.format="%m/%d/%Y", adjust.for.earliest.date=TRUE, plot.type=c("baseR", "SVG")[1], suppress.warnings=FALSE)
{
  ...
  if( plot.type[1] == "baseR" )
  {
    ...
    {
      if( !suppress.warnings ) .write.ewsn("No CMA plot or no base R plot were generated!\n", "warning", "map.event.coords.to.plot", "AdhereR");
      return (NULL);
    } else
    {
      # x:
  ...
  } else if( plot.type[1] == "SVG" )
  {
    ...
    {
      if( !suppress.warnings ) .write.ewsn("No CMA plot or no SVG was generated!\n", "warning", "map.event.coords.to.plot", "AdhereR");
      return (NULL);
    } else
    {
      # x:
  ...
  {
    if( !suppress.warnings ) .write.ewsn("Unknown plot type!\n", "warning", "map.event.coords.to.plot", "AdhereR");
    return (NULL);
  }
}
...
get.event.plotting.area <- function(plot.type=c("baseR", "SVG")[1], suppress.warnings=FALSE)
{
  ...
  if( plot.type[1] == "baseR" )
  {
    if( is.null(lcpi) || is.null(lcpi$baseR) )
    {
      if( !suppress.warnings ) .write.ewsn("No CMA plot or no base R was generated!\n", "warning", "get.event.plotting.area", "AdhereR");
      return (NULL);
    } else
    {
...
get.plotted.events <- function(plot.type=c("baseR", "SVG")[1], suppress.warnings=FALSE)
{
  ...
  if( plot.type[1] == "baseR" )
  {
    ...
    } else
    {
      ...
      {
        if( !suppress.warnings ) .write.ewsn("No info about the plotted CMA!\n", "warning", "get.plotted.events", "AdhereR");
        return (NULL);
      } else
      {
...
get.plotted.partial.cmas <- function(plot.type=c("baseR", "SVG")[1], suppress.warnings=FALSE)
{
  ...
  if( plot.type[1] == "baseR" )
  {
    ...
    } else
    {
      ...
      {
        if( !suppress.warnings ) .write.ewsn("No partial CMAs: are you sur this is the right type of CMA and that the partial CMAs were actually plotted?\n", "warning", "get.plotted.partial.cmas", "AdhereR");
        return (NULL);
      } else
      {
...
)
{
  ...
  {
    if( !suppress.warnings ) .write.ewsn("Can only plot a correctly specified CMA object (i.e., with valid data and column names)!\n", "warning", ".plot.CMAs", "AdhereR");
    return (invisible(NULL));
  }
  ...
  if( align.all.patients && show.period != "days" ){ show.period <- "days"; if( !suppress.warnings ) .write.ewsn("When aligning all patients, cannot show actual dates: showing days instead!\n", "warning", ".plot.CMAs", "AdhereR"); }
  ...
  if( length(patids) == 0 )
  {
    if( !suppress.warnings ) .write.ewsn("No patients to plot!\n", "warning", ".plot.CMAs", "AdhereR");
    return (invisible(NULL));
  } else if( length(patids) > max.patients.to.plot )
  {
    if( !suppress.warnings ) .write.ewsn(paste0("Too many patients to plot (",length(patids),
  ...
  } else if( inherits(cma, "CMA0") && is.null(cma$event.info) )
  {
    ...
    if( !is.null(event.info) )
    {
      ...
      {
        if( !suppress.warnings ) .write.ewsn("No events in the follow-up window: nothing to plot!\n", "warning", ".plot.CMAs", "AdhereR");
        return (invisible(NULL));
      }
    ...
    {
      if( !suppress.warnings ) .write.ewsn("Error(s) concerning the follow-up and observation windows: please see console for details!\n", "warning", ".plot.CMAs", "AdhereR");
      return (invisible(NULL));
    }
  }
  ...
  if( !inherits(cma$data[,cma$event.date.colname], "Date") )
  {
    ...
    {
      if( !suppress.warnings ) .write.ewsn("The date format must be a single string: cannot continue plotting!\n", "warning", ".plot.CMAs", "AdhereR");
      return (invisible(NULL));
    }
    ...
    {
      if( !suppress.warnings ) .write.ewsn(paste0("Not all entries in the event date \"",cma$event.date.colname,"\" column are valid dates or conform to the date format \"",cma$date.format,"\"; first issue occurs on row ",min(which(is.na(cma$data$.DATE.as.Date))),": cannot continue plotting!\n"), "warning", ".plot.CMAs", "AdhereR");
      return (invisible(NULL));
    }
  } else
  {
  ...
  if( is.cma.TS.or.SW )
  {
    ...
    {
      .write.ewsn(paste0("The minimum vertical space for the timeseries plots (plot.partial.CMAs.as.timeseries.vspace) is 5 lines, but it currently is only ",
  ...
  if( .do.R ) # Rplot:
  {
                ...
                "try-error"))
    {
      ...
      .write.ewsn(msg, "warning", ".plot.CMAs", "AdhereR");
    ...
    {
      .write.ewsn(paste0("Plotting area is too small (it must be at least ",
  ...
  if( .do.SVG ) # Close the <sgv> tag:
  {
    ...
    if( !is.null(export.formats) )
    {
      ...
      if( "html" %in% export.formats )
      {
        ...
        {
          .write.ewsn("Cannot load the CSS template -- please reinstall the AdhereR package!\n", "warning", ".plot.CMAs", "AdhereR");
          assign(".last.cma.plot.info", .last.cma.plot.info, envir=.adherer.env); # save the plot infor into the environment
          return (invisible(NULL));
        }
        ...
        {
          .write.ewsn("Cannot load the JavaScript template -- please reinstall the AdhereR package!\n", "warning", ".plot.CMAs", "AdhereR");
          assign(".last.cma.plot.info", .last.cma.plot.info, envir=.adherer.env); # save the plot infor into the environment
          return (invisible(NULL));
        }
        ...
        {
          .write.ewsn("Cannot load the HTML template -- please reinstall the AdhereR package!\n", "warning", ".plot.CMAs", "AdhereR");
          assign(".last.cma.plot.info", .last.cma.plot.info, envir=.adherer.env); # save the plot infor into the environment
          return (invisible(NULL));
        }
