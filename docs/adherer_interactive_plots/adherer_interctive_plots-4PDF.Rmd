...
```{r , echo=FALSE, warning=FALSE, out.width = "100%", fig.align='center', fig.cap="\\textbf{Overview of the User Interface.} Screenshot (App is running in Firefox on macOS 10.13) of some of the main UI elements (dotted black ellipses or rectangles identified with black numbers). \\textbf{1} is a button that opens a box giving information about the App. \\textbf{2} is a button that exist the App cleanly (i.e., stops and disconnects the session). \\textbf{3} is the main plotting area which displays the current plot. \\textbf{4} shows some of the UI elements controlling the plot (element 4) size. \\textbf{5} is the area where various parameters can be modified. \\textbf{6} opens UI elements specific for saving the current plot to file. \\textbf{7} shows the \\texttt{R} code that can be used to generate the current plot. \\textbf{8} gives access to UI controls that allow the computation of the current CMA for many more patients and saving the results to file. \\textbf{9} displays messages, warnings or errors that might have been geenrated while constructing the current plot."}
knitr::include_graphics("shiny-overview.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "100%", fig.align='center', fig.cap="\\textbf{Saving the current plot to file.} A zoom-in of the new controls (element \\textbf{10}) revealed by turning the \\textbf{Save plot!} switch (UI element \\textbf{6}) \\textbf{ON}. Also visible is an explanatory \\textit{tooltip}."}
knitr::include_graphics("shiny-saveplot.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "10cm", fig.align='center', fig.cap="\\textbf{Viewing the \\texttt{R} code that can generates the current plot.}"}
knitr::include_graphics("shiny-rcode.jpg")
```
...
get.data.for.patients.fnc <- function(patientid, 
                                      d, 
                                      idcol, 
                                      cols=NA, 
                                      maxrows=NA) 
...
```{r , echo=FALSE, warning=FALSE, out.width = "100%", fig.align='center', fig.cap="\\textbf{Selecting patients \\textit{individually} for the computation of CMA.}"}
knitr::include_graphics("shiny-computecma-individually.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "100%", fig.align='center', fig.cap="\\textbf{Selecting patients \\textit{by range} of positions in a list for the computation of CMA.} In this example, we ordered the patients descreasingly by ID (using the combobox \\textbf{13}), resulting in a mapping of positions (\\#) to ID as shown in the list \\textbf{14}, where patient with ID \\textit{100} is on position 1, patient \\textit{99} on position 2, etc. The sliders \\textbf{15} define the range of positions (\\#) 3 to 24, which means that we selected patients with IDs \\textit{98}, \\textit{97}, \\textit{96} ... \\textit{78} and \\textit{77}."}
knitr::include_graphics("shiny-computecma-range.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "10cm", fig.align='center', fig.cap="\\textbf{Starting, stopping and watchin the progress of CMA computation for several patients.} The list of patients is given in UI element \\textbf{16}*, and the progress of the computation is tracked by theprogress bar and individual success report (UI elements \\textbf{17}). The button \\textbf{Save results (as TSV)} allows the user to select a file where the results will be exported as a TAB-separated CSV file."}
knitr::include_graphics("shiny-computecma-dialog.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "10cm", fig.align='center', fig.cap="\\textbf{Basic information about the current dataset.} Here, \\texttt{med.events}, showing also the five important columns."}
knitr::include_graphics("shiny-infodataset.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "100%", fig.align='center', fig.cap="\\textbf{Folding and unfolding the contents of a section.} The section \\textbf{Follow-up window (FUW)} with content folded (hidden) on the left, and unfolded (visible) on the right."}
knitr::include_graphics("shiny-foldsections.jpg")
```

  - some sections may appear or disappear depending on other circumstances, such as the type of CMA selected (e.g., <b style='color: darkblue'>Define episodes</b> exists only for *CMA per episodes*) or the optional columns being defined (e.g., <b style='color: darkblue'>Show dose</b> exists only if the daily dose column was defined).
...
```{r , echo=FALSE, warning=FALSE, out.width = "100%", fig.align='center', fig.cap="\\textbf{Defining sliding windows.} Here we show 90-days sliding windows lagging by 60 days and starting right at the begining of the observation window. The regularly spaced bars at the top of the plot represent the sliding window, each with its own CMA estimate (here, \\texttt{CMA9}). The histogram on the left (which can be toggled to a barplot) shows the distribution of the CMA estimates across the sliding windows."}
knitr::include_graphics("shiny-slidingwindows.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "100%", fig.align='center', fig.cap="\\textbf{Vertically aligning multiple patients.} The top panel shows two patients (with IDs \\textit{1} and \\textit{15}) plotted using the actual dates of their events (as difference between the earilest event and their own) versus the same two patients aligned vertically relative to each other."}
knitr::include_graphics("shiny-alignpatients.jpg")
```
...
This section is shown only a *daily dose* column is defined for the current data source and controls how the dose is visually shown (if at all):
...
```{r , echo=FALSE, warning=FALSE, out.width = "6cm", fig.align='center', fig.cap="\\textbf{The possible line styles} used for the event lines."}
knitr::include_graphics("shiny-linestyles.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "10cm", fig.align='center', fig.cap="\\textbf{The possible point symbols} used to mark the start and the end of an event."}
knitr::include_graphics("shiny-pointsymbols.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "100%", fig.align='center', fig.cap="\\textbf{Starting the App with no data source.} The highlighted panel on the left (UI element \\textbf{18}) is now open at the \\textbf{Data} tab, which allows the interactive selection of various types of data sources."}
knitr::include_graphics("shiny-data-empty.jpg")
```

```{r , echo=FALSE, warning=FALSE, out.width = "6cm", fig.align='center', fig.cap="\\textbf{Starting the App with no data source.} The \\textbf{Params} tab on the left now contains only a warning message (and no settings)."}
knitr::include_graphics("shiny-data-empty-params.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "6cm", fig.align='center', fig.cap="\\textbf{Selecting and \\textit{in-memory} data source.}"}
knitr::include_graphics("shiny-data-inmemory.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "10cm", fig.align='center', fig.cap="\\textbf{Selecting the \\texttt{med.events} example dataset.} Please note that the selection ca be done by looking through the list, or by typing the \\texttt{delete}/\\texttt{backspace} key and then starting to type the name of the dataset."}
knitr::include_graphics("shiny-data-inmemory-select-medevents.jpg")
```

After clicking on it, the dataset is selected and optionally available for inspection using the **Peek at dataset** button:

```{r , echo=FALSE, warning=FALSE, out.width = "10cm", fig.align='center', fig.cap="\\textbf{Peeking at (i.e., inspecting) the \\textit{in-memory} dataset \\texttt{med.events}.} This box shows basic info about the dataset, including the number of rows and columns and, for each column, its name and type, plus the first few rows."}
knitr::include_graphics("shiny-data-inmemory-peek.jpg")
```
...
```{r , echo=FALSE, warning=FALSE, out.width = "12cm", fig.align='center', fig.cap="\\textbf{Selecting the 'important' columns -- here, the one contaning the Patient IDs -- from the \\textit{in-memory} dataset `med.events`.} Please note that the list give extra summary info about the columns in the dataset (namely, its name, type, and first few values). The App automatically maps the first three columns in the dataset to the first three required columns (\\textbf{Patient ID column}, \\textbf{Event date column} and \\textbf{Event duration column}) but this is most probably wrong and no type checks are done at this time. The 'optional' columns (\\textbf{Daily dose column} and \\textbf{Treatment class column} may be left undefined by selecting the \\textit{[not defined]} value. \\textbf{Date format} is different, being a free text field where the format of the dates in the dates column must be defined (please see, for example \\url{https://www.statmethods.net/input/dates.html}, for how this format looks like)."}
knitr::include_graphics("shiny-data-inmemory-selectcolumns.jpg")
```
