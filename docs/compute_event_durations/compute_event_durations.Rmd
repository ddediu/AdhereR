---
title: 'Data preparation for computing adherence to medication in AdhereR'
...
vignette: >
  ...
  %\VignetteIndexEntry{Data preparation for computing adherence to medication in AdhereR}
...
`special.periods.method`), and any of those specified in `medication.class.colnames`.
- `ID.colname`: A *string*, the name of the column in `disp.data`,
`presc.data`, and `special.periods.data` containing the unique patient ID.
...
treatment interruptions (see `special.periods.method`).
...
- `special.periods.method`: Can be either of *continue*, *discard*,
...
# select medication class of interest and compute event durations with split.on.dosage.change = TRUE
...
dosage.changes <- event_durations[DISP.START != DATE.DISP, .(days = as.numeric(DISP.START-as.Date("2056-07-01")),
                                                             I = I)]
svg("plots/fig5a.svg", width = 8, height = 6)
...
# select medication class of interest and compute event durations with split.on.dosage.change = FALSE
event_durations_list_2 <- compute_event_durations(disp.data = durcomp.dispensing[ID == 7 & grepl("A10AB", ATC.CODE)],
                                           presc.data = durcomp.prescribing[ID == 7 & grepl("A10AB", ATC.CODE)],
                                           special.periods.data = durcomp.hospitalisation,
                                           special.periods.method = "continue",
                                           ID.colname = "ID",
...
event_durations <- event_durations_list_2$event_durations

cma0_FALSE <- CMA0(event_durations[DURATION > 0],
             ID.colname = "ID",
             event.date.colname = "DISP.START",
             event.duration.colname = "DURATION",
             event.daily.dose.colname = "DAILY.DOSE",
...
svg("plots/fig5b.svg", width = 8, height = 6)
plot(cma0_FALSE, min.plot.size.in.characters.vert = 0,print.dose = TRUE,show.legend = FALSE, highlight.followup.window = FALSE, highlight.observation.window = FALSE)
dev.off()
...
<!--html_preserve-->
<div class="ba-slider">
   <img src="plots/fig5b.svg">
   <div class="resize">
       <img src="plots/fig5a.svg">
   </div>
   <span class="handle"></span>
</div>
<a name="Figure-5"></a><strong>Figure 5: </strong>CMA0 calculated for a patient with split.on.dosage.change = TRUE (left) or FALSE (right). Numbers represent dosage in milligrams. Blue vertical lines indicate dates of dosage change. Drag the slider to reveal the differences.
<!--/html_preserve-->
...
first_presc_plot <- (72+as.numeric(first_presc-as.Date("2056-07-31"))+5)/(2*365+10+72)
...
# just for illustration, no reliable solution for plotting
abline(v = first_presc_plot )
points(x = first_presc_plot , y = 0.99, pch = 25, bg = "black")
text(x = first_presc_plot+0.005 , 0.95, "First prescription", pos = 4)
```
...
for(i in 1:nrow(TEs)){
  bottom = 0
  top = 1
  start = (as.numeric(TEs[i, "episode.start"]-head(TEs[,"episode.start"],1)) + 26+5)/375
  end = (as.numeric(TEs[i, "episode.end"]-head(TEs[, "episode.start"],1)) + 26+5)/375
  offset = 0#(min(TEs[["episode.start"]], na.rm = TRUE)-min(event_durations[["DISP.START"]], na.rm = TRUE)+5+26)/375

  rect(xleft=start+offset, xright=end+offset, ybottom=bottom, ytop=top, col = rgb(1,1,0,alpha = 0.2), border = "black", lty = "dashed", lwd = 0.1)}
points(x = (as.numeric(all_presc_events[,DATE.PRESC-as.Date("2057-01-27")])+26+5)/375 , y = rep(0.99, nrow(all_presc_events)), pch = 25, bg = "grey")
points(x = (as.numeric(presc_events[,DATE.PRESC-as.Date("2057-01-27")])+26+5)/375 , y = rep(0.99, nrow(presc_events)), pch = 25, bg = "blue")
...
for(i in 1:nrow(durcomp.hospitalisation[ID == 3])){
  ...
  bottom = 0
  top = 1
  start = as.numeric(durcomp.hospitalisation[ID == 3][[i, "DATE.IN"]]-first_event+5)/(2*365+10)
  end = as.numeric(durcomp.hospitalisation[ID == 3][[i, "DATE.OUT"]]-first_event+5)/(2*365+10)

  rect(xleft=start, xright=end, ybottom=bottom, ytop=top, col = rgb(0,0,1,alpha = 0.5), border = NULL)}
```
...
```{r, echo=FALSE, fig.show='hold', fig.cap = "**Figure 2:** CMA0 calculated for a patient with different types of special periods and special.periods.method = \"CUSTOM\". Bars at the top of the plot and shaded areas indicate special periods: Blue for 'continue' and Purple for 'carryover'.", fig.height=6, fig.width=8, out.width="100%"}
...
for(i in 1:nrow(special_episodes)){
  ...
  bottom = nrow(event_durations)/(nrow(event_durations)+1)+i/40+0.025
  top = bottom+0.025
  start = as.numeric(special_episodes[[i, "DATE.IN"]]-first_event+5)/110
  end = as.numeric(special_episodes[[i, "DATE.OUT"]]-first_event+5)/110
...
svg("plots/fig6a.svg", width = 8, height = 6)
plot(cma7, min.plot.size.in.characters.vert = 0, show.legend = FALSE, show.period = "dates")
# just for illustration, no reliable solution for plotting
for(i in 1:nrow(event_durations_list$special_periods)){
  ...
  bottom = 0
  top = 1
  start = (as.numeric(event_durations_list$special_periods[[i, "DATE.IN"]]-first_event)+5+72)/(2*365+10+72)
  end = (as.numeric(event_durations_list$special_periods[[i, "DATE.OUT"]]-first_event)+5+72)/(2*365+10+72)

  rect(xleft=start, xright=end, ybottom=bottom, ytop=top, col = rgb(0,0,1,alpha = 0.5), border = NULL)
}
dev.off()

svg("plots/fig6b.svg", width = 8, height = 6)
...
<!--html_preserve-->
<div class="ba-slider">
   <img src="plots/fig6b.svg">
   <div class="resize">
       <img src="plots/fig6a.svg">
   </div>
   <span class="handle"></span>
</div>
<a name="Figure-6"></a><strong>Figure 6: </strong>CMA7 calculated with event.date.colname = "DATE.DISP" (left) and event.date.colname = "DISP.START" (right) for an observation period starting and ending during special periods with carryover. Drag the slider to reveal the differences.
<!--/html_preserve-->
...
<script>(function(t,e,s,o){var n,a,c;t.SMCX=t.SMCX||[],e.getElementById(o)||(n=e.getElementsByTagName(s),a=n[n.length-1],c=e.createElement(s),c.type="text/javascript",c.async=!0,c.id=o,c.src=["https:"===location.protocol?"https://":"http://","widget.surveymonkey.com/collect/website/js/tRaiETqnLgj758hTBazgd7pJaijHH5as88m1QTaCa8AkTTbdOpApBZm_2BzZwW21bT.js"].join(""),a.parentNode.insertBefore(c,a))})(window,document,"script","smcx-sdk");</script>
<!--/html_preserve-->
