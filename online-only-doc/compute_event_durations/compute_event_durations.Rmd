...
# select durations ending at the start of a special period
dt1 <- na.omit(special_periods[event_durations], cols = "DATE.IN")
...
# select durations beginning at the end of a special period
dt2 <- na.omit(special_periods[event_durations], cols = "DATE.OUT")
dt2 <- dt2[,c(event_durations_list$ID.colname, "DATE.IN", "DATE.OUT", "ATC.CODE", "SPECIAL.DURATION", ".prune.event"), with = FALSE] # only keep necessary columns, including flag for pruning
...
# change column names
setnames(covered_periods,
         old = c("DATE.IN", "SPECIAL.DURATION"),
         new = c("DISP.START", "DURATION"))

# append covered periods to event durations dataset
event_durations_list$event_durations <- rbind(event_durations, covered_periods, fill = TRUE)
...
event_durations <- event_durations[DURATION > 0 & ID == 3 & grepl("J01EE01", ATC.CODE)]
