...
During special periods and treatment interruptions, medication use may differ from daily life. `special.periods.mapping` tells `AdhereR` what to do during such periods. Similarly, `trt.interruption` specifies handling of treatment interruptions (periods without prescription). There are 4 options that can be set globally:
...
```{r, echo=TRUE, fig.show='hold', fig.cap = "<a name=\"Figure-1\"></a>**Figure 1:** CMA0 calculated for a patient with trt.interruption = \"carryover\"", fig.height=6, fig.width=8, out.width="100%"}
...
```{r, echo=TRUE, fig.show='hold', fig.cap = "<a name=\"Figure-2\"></a>**Figure 2:** CMA0 calculated for a patient with different types of special periods and special.periods.mapping = \"CUSTOM\". Bars at the top of the plot and shaded areas indicate special periods: Blue for 'continue' and Purple for 'carryover'.", fig.height=6, fig.width=8, out.width="100%"}
...
```{r, echo=TRUE, fig.show='hold', fig.cap = "<a name=\"Figure-3\"></a>**Figure 3:** CMA7 calculated for a patient with force.init.presc = TRUE", fig.height=6, fig.width=8, out.width="100%"}
...
```{r, echo=TRUE, fig.show='hold', fig.cap = "<a name=\"Figure-6\"></a>**Figure 6:** CMA7 calculated with event.date.colname = \"DATE.DISP\" (left) and event.date.colname = \"DISP.START\" (right) for an observation period starting and ending during special periods with carryover.", fig.height=6, fig.width=8, out.width="100%"}
event_durations_list <- compute_event_durations(disp.data = durcomp.dispensing[ID == 3 & grepl("J01EE01", ATC.CODE)],
                                           presc.data = durcomp.prescribing[ID == 3 & grepl("J01EE01", ATC.CODE)],
...
Without further processing of event_durations for CMA computations, special periods will appear as gaps, possibly leading to underestimation of implementation or even assumption of discontinuation and non-persistence. To consider such periods as covered, they can be added to the `event_durations` dataset. This can be achieved by merging the special periods with the `event_durations` dataset. This should be done after pruning with `prune_event_durations`. We can use rolling joins in `data.table` to identify special periods that are in proximity to already covered durations (Alex/Dan: should this be encapsulated in a separate function?):
...
```{r, echo=TRUE, fig.show='hold', fig.cap = "<a name=\"Figure-7\"></a>**Figure 7:** CMA0 calculated for a patient with trt.interruption = \"carryover\", carryover events pruned and special durations covered.", fig.height=6, fig.width=8, out.width="100%"r echo=FALSE}
...
During periods without prescriptions (treatment interruptions) or certain special episodes, it might not be meaninful to calculate a CMA at all. In these instances, it might be advisable to use `CMA_per_episode` with precomputed episodes, e.g. from the output of `compute_event_durations`. 

```{r}
# select medication with fixed treatment episodes
...
prescription_episodes[is.na(END.PRESC), END.PRESC := as.Date("2058-01-01")]

# calculate duration
prescription_episodes[is.na(PRESC.DURATION), PRESC.DURATION := as.numeric(END.PRESC-START.PRESC)]


# add end.episode.gap.days and episode.ID
prescription_episodes[,`:=` (end.episode.gap.days = shift(START.PRESC, n=1, type="lead")-END.PRESC,
                             episode.ID = seq(.N)), by = c(event_durations_list$ID.colname, event_durations_list$medication.class.colnames)]

prescription_episodes[is.na(end.episode.gap.days), end.episode.gap.days := 0]

# drop unnecessary columns
prescription_episodes[,`:=` (ATC.CODE = NULL,
                             UNIT = NULL,
                             FORM = NULL,
                             DAILY.DOSE = NULL)]

# change column names
setnames(prescription_episodes,
         old = c("START.PRESC", "PRESC.DURATION", "END.PRESC"),
         new = c("episode.start", "episode.duration", "episode.end"))

# compute CMA per episode

cma_episode <- CMA_per_episode(data = event_durations[DURATION > 0],
                               treat.epi = prescription_episodes,
                               CMA.to.apply = "CMA7",
                               ID.colname = "ID",
                               event.date.colname = "DISP.START",
                               event.duration.colname = "DURATION",
