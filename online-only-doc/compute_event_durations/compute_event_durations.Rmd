...
## Accounting for carryover during special periods

If special periods of type *carryover* need to be considered during CMA computations, the `event_durations` need further processing. Otherwise, these periods will appear as gaps, possibly leading to underestimation of implementation or even assumption of discontinuation and non-persistence. On the other hand, *carryover* during special periods may lead to overestimation of implementation, e.g. if patients get a refill after discharge from hospital and don't continue to use their previous supply. Likewise, it may also lead to overestimation of persistence, e.g. when patients do in fact discontinue treatments at the end of a special period. 

To detect whether new dispensing events occur shortly after the end of a special period in spite of a remaining supply, `AdhereR` offers the `prune_event_durations` function. It accepts the raw list output of `compute_event_durations` and additional arguments to specify event durations that need to be removed:

- `data`: a list, the output of `compute_event_durations`
- `days.within.out.date.1`: event durations from before the special period are removed if there is a new dispensing event within the number of days after the end of a special period
...
```{r}
prune_event_durations <- function(data, days.within.out.date.1, days.within.out.date.2){
  ...
  setkeyv(special_periods, cols = c(data$ID.colname, "join_date"))
  
  disp.within <- na.omit(special_periods[event_durations, roll = days.within.out.date.1], cols = c("DURATION", "DATE.OUT"))
  
  disp.within.carryover <- disp.within[get(data$disp.date.colname) < join_date & get(data$disp.date.colname) != DISP.START]
  disp.within.new <- disp.within[-disp.within[get(data$disp.date.colname) < join_date & get(data$disp.date.colname) != DISP.START, which = TRUE]]
  
  disp.remove.1 <- disp.within.carryover[disp.within.new, on = c(data$ID.colname, data$medication.class.colnames, "DATE.OUT"), nomatch=0]
