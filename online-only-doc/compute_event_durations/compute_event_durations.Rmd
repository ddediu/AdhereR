...
This vignette describes the functions `compute_event_durations` and its arguments. We also discuss the computation of adherence with the output of `compute_event_durations`, introducing the functions `prune_event_durations`, `cover_special_periods` and `time_to_initiation`. We use the provided example datasets included in the dataset to illustrate the various options and their impact on the calculated durations.
...
- disp.data A \emph{\code{data.frame}} or \emph{\code{data.table}} containing
...
- presc.data A \emph{\code{data.frame}} containing the prescribing events.
...
- special.periods.data Optional, \emph{\code{NULL}} or a \emph{\code{data.frame}}
...
- presc.duration.colname A \emph{string}, the name of the column in
\code{presc.data} containing the duration of the prescription as \code{numeric}
or \code{NA} if duration is unknown.
- visit.colname A \emph{string}, the name of the column in
...
- force.init.presc \emph{Logical}. If \code{TRUE} advance the date of the
...
- force.presc.renew \emph{Logical} or \emph{string}. If \code{TRUE} require
...
- trt.interruption \emph can be either of \emph{"continue"}, \emph{"discard"},
...
- split.on.dosage.change \emph{Logical} or \emph{string}. If \code{TRUE}
...
- suppress.warnings \emph{Logical}, if \code{TRUE} don't show any warnings.
- return.data.table \emph{Logical}, if \code{TRUE} return a
\code{data.table} object, otherwise a \code{data.frame}.
- progress.bar \emph{Logical}, if \code{TRUE} show a progress bar.
...
```{r, echo=FALSE, fig.show='hold', fig.cap = "<a name=\"Figure-4\"></a>**Figure 4:** CMA0 calculated for a patient with force.presc.renew = TRUE", fig.height=6, fig.width=8, out.width="100%"}
...
```{r, echo=FALSE, fig.show='hold', fig.cap = "<a name=\"Figure-5\"></a>**Figure 5:** CMA0 calculated for a patient with split.on.dosage.change = TRUE", fig.height=6, fig.width=8, out.width="100%"}
...
- `date.forma`: a *string* with the date format.
...
-	functions presented here are `compute_event_durations`, `prune_event_durations`, `cover_special_periods`, and `time_to_initiation` to prepare data for calculation of TE, CMA, and eventually `time_to_inititation`
