...
This section is shown only for **CMA per episodes** and concerns the way treatment episodes are defined:
  ...
  - **Append gap?**: should (part of) the maximum permissible gap be added to the episodes with a gap larger than this maximum? If "no" (the default), nothing is added, if "yes" then 100% of the maximum permissible gap is appended at the end of the episodes, and if "partially...", a new sliding control appears, allowing the user to chose the percent of the maximum permissible gap to be added, between 0% (nothing) to 100% (all of it)
...
For example, when using the `MySQL` database described in the vignette [*Using AdhereR with various database technologies for processing very large datasets*](https://cran.r-project.org/web/packages/AdhereR/vignettes/AdhereR-overview.html) in package `AdhereR`, we can create, for example, a `view` (named `testview`) that brings together these data in the needed format with the following `SQL` commands in `MySQL Workbench`:
