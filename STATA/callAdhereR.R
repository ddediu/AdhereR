...
# 2. STATA calls R using the "shell" mechanism: . shell Rscript --vanilla PATH_TO_ADHERER/callAdhereR.R PATH_TO_WHERE_DATA_AND_RESULTS_ARE
...
if( !is.null(results) )
{
  # save the resuts (possibly applying conversions):
  ...
  {
    # CMAs:
    write.table(.apply.export.conversions(results$CMA), paste0(folder.path,"./CMA.csv"), row.names=FALSE, col.names=TRUE, sep="\t", quote=FALSE);
    # event info:
    ...
    {
      write.table(.apply.export.conversions(results$event.info), paste0(folder.path,"./EVENTINFO.csv"), row.names=FALSE, col.names=TRUE, sep="\t", quote=FALSE);
    }
